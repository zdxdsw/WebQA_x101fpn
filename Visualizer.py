...
class Visualizer:
    ...
    def draw_instance_predictions(self, predictions):
        ...
        """
        f = open("genome_1600_400_20_obj_vocab.txt", "r")
        lines = f.readlines()
        THING_CLASSES = [l.strip() for l in lines]
        ...
        if self._instance_mode == ColorMode.SEGMENTATION and self.metadata.get("thing_colors"):
            colors = [
                self._jitter([x / 255 for x in self.metadata.thing_colors[c//2]]) for c in classes
            ]
            alpha = 0.8
        else:
            colors = None
